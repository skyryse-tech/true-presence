"""
Unified Attendance System - Single Person or Multi-Person Mode
Real-time face recognition for attendance with mode selection
"""
...
class AttendanceDatabase:
    """Database for storing attendance records with both single and multi-person support"""
    ...
    def add_person(self, name: str, photo_count: int = 0):
        ...
        try:
            cursor.execute('''
                INSERT OR REPLACE INTO persons (name, enrolled_date, photo_count)
                VALUES (?, ?, ?)
    ...
    def mark_attendance(self, person_name: str, confidence: float, photo_path: str = None, 
                       face_position: str = None, detection_mode: str = "single"):
    ...
    def get_today_attendance(self):
        ...
        today = datetime.now().date()
        cursor.execute('''
            SELECT person_name, timestamp, confidence, face_position, detection_mode
            FROM attendance 
    ...
    def get_all_enrolled_persons(self):
        ...
        cursor.execute('''
            SELECT name, total_attendance, last_seen 
            FROM persons 
            ORDER BY name
        ''')
...
class UnifiedFaceRecognizer:
    ...
    def detect_all_faces(self, frame: np.ndarray) -> List[Tuple[int, int, int, int]]:
        """Detect all faces in the frame (for multi-person mode)"""
        try:
...
def select_mode():
    ...
    print("🎭 TRUE PRESENCE - UNIFIED ATTENDANCE SYSTEM")
    print("=" * 70)
...
def main():
    ...
    if not test_photos_dir.exists() or not any(test_photos_dir.iterdir()):
        ...
        print("   4. python unified_attendance_system.py # Start attendance")
        return False
